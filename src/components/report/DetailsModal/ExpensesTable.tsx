        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gray-100 text-gray-700">
              {["ลำดับ","ไอดีรายการ","วันเวลาที่บันทึก","รายการ","จำนวน","หน่วย"].map((h) => (
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-gray-400">ไม่มีข้อมูล</td>
              </tr>
            ) : (
              rows.map((r, i) => {
                ...
                return (
                  <tr key={r.id} className="border-b">
                    ...
                    <td className="px-3 py-2">{dt.date} {dt.time}</td>
